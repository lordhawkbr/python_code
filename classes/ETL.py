...
class ETL:
    def __init__(self):
        # PARAMETROS DE CONEXAO COM O BANCO
            ...
            if os.path.exists(self.dataframes):
    ...
    async def main(self):
        if self.mountMainDF():
            ...
            dm_doencas["CID_10"] = dm_doencas["CID_10"].astype(str)
            dm_doencas["CID_10"] = dm_doencas["CID_10"].str.replace(".","")
            dm_doencas["CID_10"] = dm_doencas["CID_10"].apply(lambda value: value if value == "0000" else value.replace("-", " "))
