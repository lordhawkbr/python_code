...
class ETL:
    ...
    def mountMainDF(self):
        ...
        self.dbFuncs.insertLog("Dataframe principal montado!")

    def sortAndDrop(self,columns, columnIndex):
        tempDf = self.mainDF.copy()
        tempDfC = tempDf.columns
        return tempDf[tempDf.columns[[i for i, value in enumerate(tempDfC) if value in columns]]].sort_values(by=columnIndex, ascending=False).drop_duplicates(subset=columnIndex).reset_index(drop=True)
    ...
    async def main(self):
        ...
        self.mainDF["CID_10"] = self.mainDF["CID_10"].astype(str)
        self.mainDF["CID_10"] = self.mainDF["CID_10"].str.replace(".","")
        self.mainDF["CID_10"] = self.mainDF["CID_10"].apply(lambda value: value if value == "0000" else value.replace("-", " "))
        ...
        c_profissoes = ["CBO","Ocupacao"]
        c_doencas = ["CID_10", "Doenca"]
        c_empregadores = ["CNAE2_0_Empregador","CNAE2_0_Empregador_1"]
        c_localidades = ["Munic_Empr","UF_Munic_Empregador","CIM"]
        ...
        dm_agentes = self.sortAndDrop(c_agentes,c_agentes[0])
        dm_profissoes = self.sortAndDrop(c_profissoes,c_profissoes[0])
        dm_doencas = self.sortAndDrop(c_doencas,c_doencas[0])
        dm_localidades = self.sortAndDrop(c_localidades, c_localidades[2])
        dm_tipo_lesao = self.sortAndDrop(c_tipo_lesao,c_tipo_lesao[0])
        dm_partes_corpo = self.sortAndDrop(c_partes_corpo,c_partes_corpo[0])
        dm_acidentes = self.sortAndDrop(c_acidentes, c_acidentes)
        dm_empregadores = self.sortAndDrop(c_empregadores,c_empregadores[0])
